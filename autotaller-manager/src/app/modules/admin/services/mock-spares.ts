...
export class MockSpareService {
  private spares: Spare[] = [
    { id: 1, code: 'SP001', description: 'Filtro de aire', stockQuantity: 10, unitPrice: 50000 },
    { id: 2, code: 'SP002', description: 'Aceite 20W50', stockQuantity: 20, unitPrice: 35000 },
  ];
  ...
  create(spare: Omit<Spare, 'id'>): Observable<Spare> {
    const id = Math.max(...this.spares.map(s => s.id), 0) + 1;
    const newSpare: Spare = { id, ...spare };
  ...
  update(id: number, spare: Spare): Observable<Spare> {
    ...
    if (index !== -1) this.spares[index] = spare;
    return of(spare).pipe(delay(300));
  }
